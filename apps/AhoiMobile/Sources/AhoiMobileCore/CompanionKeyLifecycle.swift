...
public enum CompanionKeyLifecycleStatus: Equatable, Sendable {
    ...
    public var isRotationPending: Bool {
...
public struct CompanionSyncRuntimeActivation: Sendable {
    public let status: CompanionKeyLifecycleStatus
    public let runtime: CompanionCloudKitRuntime?
    private let discardHandler: @Sendable () async -> Void

    public init(
        status: CompanionKeyLifecycleStatus,
        runtime: CompanionCloudKitRuntime?,
        discardHandler: (@Sendable () async -> Void)? = nil
    ) {
        self.status = status
        self.runtime = runtime
        self.discardHandler = discardHandler ?? {
            guard let provider = runtime?.provider else { return }
            provider.setEventDrivenSyncHandler(nil)
            await provider.cancel()
        }
    }

    public func discardRuntime() async {
