...
public actor KeychainCompanionPayloadKeyStore:
    CompanionPayloadKeyLifecycleStoring,
    CompanionPayloadKeyRotationStoring {
    ...
    /// Installs a synchronizable rotation key without changing the bootstrap
    /// candidate journal. A retry reads and validates the existing key, while a
    /// concurrent duplicate is accepted only when its bytes match exactly.
    public func installCanonicalKey(
        version: UInt32,
        ...
        let current = try readRequiredItem(
            kind: .canonical,
        ...
        if let existing = try readItem(kind: .canonical, version: version) {
            guard existing.count == 32 else {
                throw CompanionPayloadKeyStoreError.invalidKeyLength
            }
            guard !constantTimeEqual(existing, current) else {
                throw CompanionPayloadKeyStoreError.rotationKeyUnchanged
            }
            return
        }
        let generated = try generator()
        ...
        guard !constantTimeEqual(generated, current) else {
            throw CompanionPayloadKeyStoreError.rotationKeyUnchanged
        }
        do {
            try addItem(generated, kind: .canonical, version: version)
        } catch CompanionPayloadKeyStoreError.keychainStatus(errSecDuplicateItem) {
            let existing = try readRequiredItem(
                kind: .canonical,
                version: version
            )
            guard constantTimeEqual(existing, generated) else {
                throw CompanionPayloadKeyStoreError.splitKeyPrevented
            }
        }
        let readback = try readRequiredItem(kind: .canonical, version: version)
        guard readback.count == 32 else {
            throw CompanionPayloadKeyStoreError.invalidKeyLength
        }
        guard constantTimeEqual(readback, generated) else {
            throw CompanionPayloadKeyStoreError.splitKeyPrevented
        }
    }

    /// Idempotent deletion is intentionally a separate primitive. The rotation
    /// coordinator calls it only after persisting a complete remote readback
    /// acknowledgement for every scheduled record.
    public func removeCanonicalKey(version: UInt32) throws {
        guard version > 0 else {
            throw CompanionPayloadKeyStoreError.invalidConfiguration
        }
        try deleteItem(kind: .canonical, version: version)
    }
