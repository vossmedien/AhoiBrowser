        ...
        NavigationStack {
            VStack(spacing: 14) {
                ...
                if !addressText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ||
                    !browser.searchOpenTabs(addressText).isEmpty ||
                    !companionModel.searchResults.isEmpty {
                    List {
                        let trimmedInput = addressText.trimmingCharacters(
                            in: .whitespacesAndNewlines
                        )
                        if !trimmedInput.isEmpty {
                            Section {
                                Button(action: commitAddress) {
                                    Label {
