...
private enum AccountContinuityPersistenceOutcome {
    case verified(accountMismatch: Bool)
    case persistenceFailed(CloudKitPersistenceDecision)
}
...
extension CloudKitSyncProvider {
    ...
    private func performAccountContinuityVerification(
        ...
        let outcome = try statusLock.withLock {
            () -> AccountContinuityPersistenceOutcome in
            ...
            if let known = lastKnownAccountIdentifier, known != currentIdentifier {
                ...
                if decision.requiresTransportBlock {
                    zoneRecoveryPending = true
                    return .persistenceFailed(decision)
                }
                return .verified(accountMismatch: true)
            }
            ...
            return .verified(accountMismatch: accountTransitionPending)
        }
        let accountMismatch: Bool
        switch outcome {
        case let .verified(mismatch):
            accountMismatch = mismatch
        case let .persistenceFailed(decision):
            markStatePersistenceFailure()
            guard let primaryError = decision.primaryError else {
                throw CloudKitSyncProviderError.unavailable
            }
            throw primaryError
        }
    ...
    /// Persists both halves of the account-switch boundary. Clearing the
    /// serialized engine state and recording the pending confirmation are
    /// independent writes, so both are attempted even when one fails.
    @discardableResult
    func persistAccountTransitionBoundary() -> Bool {
        let clearDecision = CloudKitStatePersistence.clearEngineState(in: stateStore)
        var persistenceFailed = clearDecision.requiresTransportBlock
        do {
            try updateSafetyState(accountTransitionPending: true)
        } catch {
    ...
    /// A missing or deleted custom zone must be durable before another event
    /// can advance the engine token. Persistence failure therefore closes the
    /// transport gate instead of being treated as a recoverable notification.
    @discardableResult
    func persistZoneRecoveryRequirement() -> Bool {
        do {
