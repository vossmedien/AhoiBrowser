...
Status: the local-first companion and macOS CKSyncEngine transport bridge are
implemented. An entitled, signed two-device CloudKit roundtrip remains an
external release gate.

The companion is a native SwiftUI app for iOS/iPadOS 17 and later. It is not a browser
replacement and does not embed a WebView. The checked-in implementation lives
under `apps/AhoiCompanion/` and can also be opened as the generated
`AhoiCompanion.xcodeproj`. Its executable target is intentionally usable
...
Companion uses the analogous `AhoiCompanion.entitlements.template`; its sync
...
`CompanionRootView` uses `NavigationSplitView`, so the same view adapts from
iPhone navigation to iPad split presentation. It displays:
...
Saved links and the primary remote-tab row call SwiftUI's system `openURL`
action. The app
does not instantiate a `WKWebView`, Chromium view, or alternate cookie/session
store. Initial and five-minute periodic sync keep CKSyncEngine's automatic
...
cd ../../apps/AhoiCompanion
swift test --jobs 1 --disable-index-store
swift build --target AhoiCompanionCore --jobs 1 --disable-index-store \
  --sdk "$(xcrun --sdk iphoneos --show-sdk-path)" --triple arm64-apple-ios26.5
swift build --target AhoiCompanionApp --jobs 1 --disable-index-store \
  --sdk "$(xcrun --sdk iphoneos --show-sdk-path)" --triple arm64-apple-ios26.5
```

The package tests exercise model validation, local persistence, local search,
...
1. Apple Developer Team ownership and final macOS/iOS bundle IDs;
...
5. signed, installed Mac and iOS builds using one authenticated iCloud test
   account;
...
Until those artifacts exist, the native app is `LOCAL_BUILD_PASS` and the
