...
// Immutable row data. `stable_id` is namespaced by result kind so a running
// saved page and its durable tree node can never alias in the native list.
// Command activation keeps Chromium/Ahoi's original stable identity; recently
// closed activation keeps only TabRestoreService's opaque SessionID.
...
class SidebarDiscoveryModel final : public CommandServiceObserver,
 ...
 public:
  ...
  std::vector<SidebarDiscoveryItem> RecentlyClosed(
      size_t max_results) const;
  ...
  void TabRestoreServiceChanged(
      sessions::TabRestoreService* service) override;
  void TabRestoreServiceLoaded(
      sessions::TabRestoreService* service) override;
